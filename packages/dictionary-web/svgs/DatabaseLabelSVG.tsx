import { SvgIcon, SvgIconProps } from "@mui/material";
import { FC } from "react";

const DatabaseLabelSVG: FC<SvgIconProps> = ({ sx, ...props }) => {
  return (
    <SvgIcon
      {...props}
      sx={{
        ...sx,
        width: "80px",
        height: "20px",
      }}
      width="68"
      height="15"
      viewBox="0 0 68 15"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <rect width="68" height="15" fill="url(#pattern2)" />
      <defs>
        <pattern
          id="pattern2"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use
            xlinkHref="#image0_1_144"
            transform="scale(0.0147059 0.0666667)"
          />
        </pattern>
        <image
          id="image0_1_144"
          width="68"
          height="15"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </SvgIcon>
  );
};

export default DatabaseLabelSVG;
